...
local function __TargetFrame_UpdateAuras(self)
	...
	local frameName = self.buffFrame:GetName();
	...
	local name, rank, icon, count, dispelType, duration, expires, isMine, isStealable;
	...
	local maxShownBuffs = 40
	local maxShownDebuffs = 40
	local onlyShowRaidBuffs = nil
	local onlyShowDispellableDebuffs = nil
	-- local tournamentStyle = ArenaLiveCore:GetDBEntry(self.unitFrame.addonName, "Auras/TournamentFilter"); -- DO NOT USE THIS FOR NORMAL UNITFRAMES
	...
	while frameID <= NUM_MAX_BUFFS do
		...
		frameName = frameName .. "Icon" .. frameID;
		frame = _G[frameName];
		
		-- Retrieve buff info for the unit
		-- name, rank, icon, count, duration, expires, isMine = UnitBuff(unit, auraID, filter);
		name, rank, icon, count, duration, expires, isMine, isStealable = UnitBuff(unit, auraID);
		if ( frameID <= maxShownBuffs and icon ) then
			...
			if ( Aura:ShouldShowBuff(self, unit, auraID, filter) ) then
				...
				if ( not unitIsPlayer and isStealable ) then
	...
	while frameID <= NUM_MAX_DEBUFFS do
		...
		name, rank, icon, count, dispelType, duration, expires, isMine = UnitDebuff(unit, auraID);
...
function Aura:AddFrame (auraFrame, buffFrame, debuffFrame, buffTemplate, debuffTemplate, unitFrame)
	...
	auraFrame.Update = __TargetFrame_UpdateAuras;
...
function Aura:UpdateAuraPositions(auraFrame, auraType, numAuras, maxRowWidth)
	...
	local normalIconSize = 21
	local largeIconSize = 21
